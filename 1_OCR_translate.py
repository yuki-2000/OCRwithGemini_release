...
- **欠落の禁止**: 文中のいかなるセクション、段落、一文も省略したり要約したりせず、全文を翻訳してください。特に文書の末尾にある**「索引（Index）」「参考文献（References）」「付録（Appendix）」**なども、勝手に翻訳を省略せず、すべて日本語に翻訳して出力してください。**ただし、後述の「除外事項」に指定されている繰り返しヘッダーやフッター、規格番号などは本文ではないため、本ルールの対象外とし、必ず除外してください。**
- **情報の完全性**: 特に大きな数式のブロック（積分、分数、行列などが含まれるもの）は、最も重要な情報です。 これらが連続する場合でも、一つ一つを独立した数式としてすべて書き出してください。「〜（以下同様の数式が続く）」といった要約は絶対に禁止します。
- **除外事項**: ページ番号、リピートされるヘッダー情報（文書名、規格番号、章名、ダウンロード元のクレジット表記（例: `Downloaded from...`）など）、フッター、の抽出・翻訳は不要です。特に、各ページの上部余白に繰り返し現れるヘッダー情報（文書名、規格番号[例: MIL-M-8856B, AMS-xxx, JIS-xxx等]、章名、セキュリティ区分など）は、最初の1ページ目のタイトルとして出現する場合を除き、2ページ目以降の繰り返しはすべて完全に無視し、マークダウン内に出力しないでください。
...
- 翻訳する際は以下のルールを必ず守ってください
   ...
   * **ページ区切りの禁止**: htmlとしてシームレスにスクロールして閲覧するため、ページとページの境界を示す `---`（水平線）や `***` などの記号、または「Page 2」といった区切りテキストは、**いかなる理由があっても一切（1箇所たりとも）出力しないでください。** ページ間の結合部は、単純な改行または空行のみで処理し、完全にフラットで地続きな1本の長いMarkdownとして出力してください。AIの標準的な出力癖による自動挿入も徹底して抑制してください。
...
- 数式はtex形式でインラインもしくはブロック数式で表現し、以下のルールを守ってください。
    ...
    * JSON形式の出力の中にMarkdown形式で出力があるため、数式内のすべてのバックスラッシュ `\` は、JSON文字列として必ず2つ重ねて `\\` とエスケープして出力してください。例えば `\frac` は JSON 内では必ず `\\frac` と、`\bullet` は `\\bullet` と、`\theta` は `\\theta` と記述してください。特に `\frac`(分数)、`\bar`(バー)、`\bullet`(中黒)、`\text`(テキスト)、`\theta`(シータ)、`\tan`(タンジェント) のように、コマンドの頭文字が `f`,`b`,`t`,`n`,`r` で始まるものは、エスケープを忘れると JSON のパース時にバックスペースや改ページ等の制御文字に化けて完全に破損します。1つのバックスラッシュ `\` を数式内に裸で出力することは、いかなる場合も固く禁止します。
